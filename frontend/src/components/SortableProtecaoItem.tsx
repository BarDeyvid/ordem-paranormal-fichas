...
export interface SortableProtecaoItemProps {
...
export const SortableProtecaoItem: React.FC<SortableProtecaoItemProps> = ({
  item,
  isExpanded,
  toggleExpandir,
  removerProtecao,
  ...
  const hasProficiencia =
    protecao.Proficiencia === 'Nenhuma' || proficienciasTotais.includes(protecao.Proficiencia);
  ...
  return (
    <div
      ...
      className={`rounded border border-l-4 border-l-blue-700 transition-colors ${
        isOverlay
          ? 'border-blue-500 bg-zinc-900 shadow-2xl scale-[1.02] opacity-90 cursor-grabbing'
          : isDragging
    ...
    >
      <div className="flex items-center gap-1 p-3">
        ...
        >
          <div className="flex flex-col gap-1 flex-1 min-w-0 justify-center">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-300 mt-0.5">
              <span>
                <span className="font-bold text-blue-400">Defesa</span>{' '}
                {(() => {
                  const extraDefMod = modsAtuais.some(
                    m => m?.Nome_Modif?.trim().toLowerCase() === 'reforçada'
                  )
                    ? 2
                    : 0;
                  ...
                  const defVal = Number(
                    String(protecao.Defesa_Protecao || '0').replace(/[^0-9.-]+/g, '')
                  );
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div
            onClick={() => toggleExpandir(id)}
            className="w-5 text-center text-zinc-500 text-xs cursor-pointer"
          >
            {isExpanded ? '▲' : '▼'}
          </div>
        </div>
      ...
      <Collapse isOpen={isExpanded}>
        <div className="border-t border-zinc-800 px-3 py-3 text-xs flex flex-col gap-2 bg-zinc-950/80">
          <div className="flex flex-col gap-1 text-xs text-zinc-300">
            <span>
              <span className="text-blue-400 font-bold">Proficiência:</span> {protecao.Proficiencia}
            </span>
            <span>
              <span className="text-blue-400 font-bold">Categoria:</span>{' '}
              {calcularCategoriaFinal(
                protecao.Categoria_Protecao,
                item.modificacoes,
            ...
            <span>
              <span className="text-blue-400 font-bold">Espaços:</span>{' '}
              {calcularEspacosFinais(
                protecao.Espacos_Protecao,
                item.modificacoes,
                modificacoesHook?.modificacoes,
