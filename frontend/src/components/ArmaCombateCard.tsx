...
export function calcularDanoMedio(
  danoStr: string,
  multCritico: number
): { normal: number; critico: number } {
...
export interface ArmaCombateCardProps {
  armaInv: ArmaInventario;
  estaExpandida: boolean;
  toggleExpandir: () => void;
  modificacoesHook: any;
  maldicoesHook: any;
  onAddMunicao?: (armaId: string) => void;
  municoesHook?: any;
  itensHook?: any;
  armasHook?: any;
}

export const ArmaCombateCard: React.FC<ArmaCombateCardProps> = ({
  armaInv,
  estaExpandida,
  toggleExpandir,
  modificacoesHook,
  ...
  const ammoMods =
    activeAmmo && Array.isArray(activeAmmo.modificacoes) ? activeAmmo.modificacoes : [];
  const ammoMalds =
    activeAmmo && Array.isArray(activeAmmo.maldicoes) ? activeAmmo.maldicoes : [];
  ...
  const modsAtivas = modsSafe
    .map(id => modificacoesHook?.modificacoes?.find((m: any) => m.Codigo_Modif === id))
    .filter(Boolean);
  const maldicoesAtivas = maldsSafe
    .map(id => maldicoesHook?.maldicoes?.find((m: any) => m.Codigo_Mald === id))
    .filter(Boolean);

  const isPontaria = ['arremesso', 'disparo', 'fogo'].some(t =>
    arma.Tipo_Arma?.toLowerCase().includes(t)
  );
  ...
  modsAtivas.forEach(m => {
    ...
    if (
      desc.toLowerCase().includes('+2 em rolagens de dano') ||
      desc.toLowerCase().includes('+2 rolagens de dano') ||
      desc.toLowerCase().includes('+2 no dano') ||
      nome.includes('cruel')
    ) {
      extrasStr += `+2`;
    }
    const match =
      nome !== 'dum dum' && nome !== 'calibre grosso' ? desc.match(/\+\s*(\d+d\d+\*?)/i) : null;
    if (match) {
      extrasStr += `+${match[1]}`;
    }
  });
    ...
    if (dtItem) {
      ...
      if (['FOR', 'AGI', 'INT', 'PRE', 'VIG'].includes(val.toUpperCase())) {
        calc =
          10 +
          (status?.peTurno || 0) +
          (atributosFinais[val.toUpperCase() as keyof typeof atributosFinais] || 0);
      } else {
        const num = Number(val);
        if (
          isNaN(num) ||
          val.toLowerCase().includes('veja') ||
          val.toLowerCase().includes('texto') ||
          val.trim() === ''
        ) {
          calc = '-';
        } else {
  ...
  if (
    ...
    (arma.Tipo_Arma?.toLowerCase() === 'corpo a corpo' ||
      arma.Tipo_Arma?.toLowerCase() === 'corpo-a-corpo') &&
    armaInv.id !== 'ataque-desarmado-virtual'
  ) {
    rawDano = rawDano.replace(/(\d+)d(\d+)/gi, (match, p1, p2) => `${Number(p1) + 1}d${p2}`);
  }
  ...
  modsAtivas.forEach(m => {
    ...
    const nome = m.Nome_Modif?.toLowerCase() || '';
    if (
      desc.includes('+2 em testes de ataque') ||
      desc.includes('+2 nas rolagens de ataque') ||
      nome.includes('certeira') ||
      nome.includes('alongad')
    ) {
      bonusAtaque += 2;
    }
  });
        ...
        <div className="pt-3 mt-3 border-t border-zinc-800 text-xs flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {municoesAcopladasList.length > 0 ? (
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {municoesAcopladasList.map((m, idx) => (
                      <button
                        key={m.id}
                        onClick={e => {
                          e.stopPropagation();
                          if (idx !== 0) {
                            armasHook?.reordenarMunicoesArma?.(armaInv.id, idx, 0);
                          }
                        }}
                  ...
                  <span className="text-zinc-500 italic text-[11px]">Nenhuma</span>
                )}
              </div>
              <button
                onClick={e => {
                  e.stopPropagation();
                  onAddMunicao?.(armaInv.id);
                }}
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mb-1">
              {parsedDano.map((pd, index) => (
                <span key={index} className="text-zinc-300">
                  ...
                  {pd.valor} <span className="text-zinc-500 text-[10px] ml-1">({pd.tipo})</span>
                </span>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 pt-2 mt-1 border-t border-zinc-800/50">
            <span className="text-zinc-300">
              <span className="font-bold text-green-400">Ataque Bônus:</span> {bonusAtaqueStr}
            </span>
            <span className="text-zinc-300">
              <span className="font-bold text-green-400">Perícia:</span> {pericia}
            </span>
            {arma.Alcance_Item && arma.Alcance_Item.trim() !== '-' && (
              <span className="text-zinc-300">
                <span className="font-bold text-green-400">Alcance:</span> {alcance}
              </span>
            )}
            <div className="flex items-center gap-1 text-zinc-300">
              ...
              <CustomSelect
                value={atributoDano}
                onChange={val => setAtributoDano(val as string)}
