...
export function calcularBonusMaldicoes(
  ...
  armas.forEach(a => {
    if (Array.isArray(a.maldicoes)) a.maldicoes.forEach(m => allMaldicoes.push({ id: m, elem: (a as any).maldicoes_elementos?.[m] }));
  });
  protecoes.forEach(p => {
    if (p.equipado && Array.isArray(p.maldicoes)) p.maldicoes.forEach(m => allMaldicoes.push({ id: m, elem: (p as any).maldicoes_elementos?.[m] }));
  });
  itens.forEach(i => {
    if (i.equipado && Array.isArray(i.maldicoes)) i.maldicoes.forEach(m => allMaldicoes.push({ id: m, elem: (i as any).maldicoes_elementos?.[m] }));
  });
  municoes.forEach(m => {
    if (Array.isArray(m.maldicoes)) m.maldicoes.forEach(m => allMaldicoes.push({ id: m, elem: (m as any).maldicoes_elementos?.[m] }));
  });
  ...
  for (let { id, elem } of allMaldicoes) {
    ...
    // Carisma
      ...
      atributos['PRE'] = (atributos['PRE'] || 0) + 1;
    }
    // Sagacidade
    ...
    // Destreza
      ...
      atributos['AGI'] = (atributos['AGI'] || 0) + 1;
    }
    // Disposição
    else if (nome === 'disposição' || nome === 'disposicao') {
      atributos['VIG'] = (atributos['VIG'] || 0) + 1;
    }
    // Pujança
    else if (nome === 'pujança' || nome === 'pujanca') {
      atributos['FOR'] = (atributos['FOR'] || 0) + 1;
    }
    // Esforço Adicional
