...
// Regex to match the whole `{hab.automatico && (...)}` block inside the header.
// It starts with `{hab.automatico && (` and ends with `)}`
// We want to delete it from the header.
const headerBlockRegex = /\{hab\.automatico && \([\s\S]*?<\/span>\s*\n\s*\)\}/;

content = content.replace(headerBlockRegex, '');

// Now we insert it inside the Collapse block.
...
if (content.includes(collapseSearch)) {
  ...
  console.log('Fixed auto badge with Regex');
} else {
