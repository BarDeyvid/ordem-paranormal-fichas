...
const search = `                                {hab.automatico && (
                                  <span
                                    title={hab.automatico.toLowerCase().trim() === 'sim' ? 'Totalmente automático — acontece sozinho' : 'Semi-automático — parte funciona automaticamente'}
...
if (content.includes(search)) {
  ...
  // Now inject it inside the Collapse!
  const collapseSearch = `<Collapse isOpen={estaExpandida}>
  ...
  if (content.includes(collapseSearch)) {
    content = content.replace(collapseSearch, collapseReplace);
    fs.writeFileSync('src/screens/Ficha/AbasPanel.tsx', content);
    console.log('Fixed auto badge in AbasPanel');
  } else {
    console.log('Failed to find collapse marker');
  }
} else {
