...
function applyToModal(file) {
  let content = fs.readFileSync(file, 'utf8');

  // We find the header `div` start and `<div className="px-3 pb-3 shrink-0">` end
  const startStr = 'className="flex  items-start justify-between gap-3 p-3"';
  ...
  const endStr = '<div className="px-3 pb-3 shrink-0">';
  ...
  const oldBlock = content.substring(divStart, endIndex);

  // Notice: no border on the classes!
  const newBlock = `<div className={\`flex justify-between gap-3 px-4 py-3 relative \${simboloImg ? 'items-stretch' : 'items-center'}\`}>
                      {/* LADO ESQUERDO */}
...
applyToModal('src/components/ModalRituais.tsx');
