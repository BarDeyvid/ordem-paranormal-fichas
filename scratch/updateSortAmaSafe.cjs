...
// 2. Destructure useRPG
if (content.includes('const { modificacoesHook } = useRPG();')) {
  content = content.replace(
...
// 3. Add obterValorVersao function
const funcCode = `
...
if (!content.includes('function obterValorVersao')) {
  content = content.replace(
...
// 4. Add Selos Paranormais logic
const searchStr = `          {item.item.Fonte_Ama && (`;
