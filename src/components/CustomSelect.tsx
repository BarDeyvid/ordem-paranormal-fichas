...
export function CustomSelect({ value, onChange, options, className = '', wrapperClassName = 'w-full', placeholder, hideIcon = false }: CustomSelectProps) {
  ...
  return (
    <div className={`relative text-left ${wrapperClassName}`} ref={containerRef}>
      ...
      <div 
        className={`absolute z-[100] mt-1 ${hideIcon ? 'w-full min-w-max left-1/2 -translate-x-1/2' : 'w-full'} overflow-hidden rounded-md border border-zinc-700/80 bg-zinc-800 shadow-xl shadow-black/50 transition-all duration-200 origin-top ${
